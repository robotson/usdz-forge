...
def createMesh(objData, geomPath, group, groupName, stage):
    ...
    if minVertexIndex == 0: # optimization
        ...
        mesh.CreateFaceVertexIndicesAttr(list(map(lambda x: x - minVertexIndex, group.vertexIndices)))
    ...
    if minUvIndex >= 0:
        if group.uvsHaveOwnIndices:
            uvPrimvar = UsdGeom.PrimvarsAPI(mesh).CreatePrimvar('st', Sdf.ValueTypeNames.TexCoord2fArray, UsdGeom.Tokens.faceVarying)
    ...
    if minNormalIndex >= 0:
        if group.normalsHaveOwnIndices:
            normalPrimvar = UsdGeom.PrimvarsAPI(mesh).CreatePrimvar('normals', Sdf.ValueTypeNames.Normal3fArray, UsdGeom.Tokens.faceVarying)
...
def usdStageWithObj(objPath, usdPath, legacyModifier, verbose=0):
    ...
    for groupName, group in objData.groups.items():
